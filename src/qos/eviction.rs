...
impl EvictionController {
    ...
    pub fn set_eviction_callback(&self, callback: EvictionCallback) {
        *self.eviction_callback.write().unwrap_or_else(|e| e.into_inner()) = Some(callback);
    }

    /// Set the compression callback
    pub fn set_compression_callback(&self, callback: CompressionCallback) {
        *self.compression_callback.write().unwrap_or_else(|e| e.into_inner()) = Some(callback);
    }
    ...
    fn compress_eligible_documents(&self, result: &mut EvictionResult) -> usize {
        let callback = self.compression_callback.read().unwrap_or_else(|e| e.into_inner());
    ...
    fn evict_document(&self, candidate: &EvictionCandidate) -> Result<usize, String> {
        let callback = self.eviction_callback.read().unwrap_or_else(|e| e.into_inner());
    ...
    pub fn unmark_protected(&self, doc_id: &str) {
        self.protected_docs.write().unwrap_or_else(|e| e.into_inner()).remove(doc_id);
    ...
    pub fn is_protected(&self, doc_id: &str) -> bool {
        self.protected_docs.read().unwrap_or_else(|e| e.into_inner()).contains(doc_id)
    }
    ...
    pub fn recent_stats(&self) -> Vec<EvictionResult> {
        self.recent_stats.read().unwrap_or_else(|e| e.into_inner()).clone()
    }
    ...
    pub fn protected_count(&self) -> usize {
        self.protected_docs.read().unwrap_or_else(|e| e.into_inner()).len()
    }
}
...
impl std::fmt::Debug for EvictionController {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EvictionController")
            ...
            .field(
                "recent_stats_count",
                &self.recent_stats.read().unwrap_or_else(|e| e.into_inner()).len(),
            )
            .finish()
    }
}
